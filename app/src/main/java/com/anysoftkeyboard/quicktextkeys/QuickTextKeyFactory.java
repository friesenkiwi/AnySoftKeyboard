...
public class QuickTextKeyFactory extends AddOnsFactory<QuickTextKey> {
    ...
    public QuickTextKeyFactory(@NonNull Context context) {
        super(context, "ASK_QKF", "com.anysoftkeyboard.plugin.QUICK_TEXT_KEY",
                ...
                R.xml.quick_text_keys, R.string.settings_default_quick_text_key_id, R.string.settings_key_ordered_active_quick_text_keys, true);
    }
    ...
    protected String getDefaultOrderedEnabledIds() {
        final List<QuickTextKey> allAddOns = getAllAddOns();
        final List<CharSequence> allIds = new ArrayList<>(allAddOns.size());
        for (QuickTextKey quickTextKey : allAddOns) {
            allIds.add(quickTextKey.getId());
        }

        return TextUtils.join(",", allIds);
    }
}
